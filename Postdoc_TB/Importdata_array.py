...
# 		self.years_in_us='',5
# 		self.birthplace='',6
# 		self.usborn=0 ,7 # default is  FB born, 1= us born
# 		self.state=0,8
# 		self.statename='',9
# 		self.age=0,10
# 		self.cycle=0,11   #### CHANGE TO BE IN CURRENT VISA STATUS
...
# 		self.alive=1,27
# 		self.tested=0,28
#		self.outgoing=0.0,29
#		self.visa=0.0,30
#		Slowdecrease 31
#		Fastdecrease 32
#		Activedecrease 33
#       HIV risk group 34
#		ltbi 10 years prob 35
#       cycle that is tested 36
#       HCW 37 : health care workers


#### Visa classes index
# "Asylees" : 1
# "Citizen" :2
# "Diplomats and other representatives" :3
# "Exchange visitors" :4
# "Families of citizens and LPR" :5
# "Other immigrant" :6
# "Refugees" :7
# "Students"  + undergrade :8
# "Temporary workers and families" :9
# "Unclassifiable" :10
## "Students"  + master :11
# "Students"  + doctoral :12
# Gren cart : 13
# M1  : 14
#F2 M2: 15
# Associate : 16
...
#@return A dictionary made based on "base_init_lines2" table. The keys are the index and values are list of (sex,race,age,citizen,years_in_us,birthplace)
...
def Make_Base_init_dict():
	...
	Initial_Person_dict={}
	cur1 = con.cursor()
	#cur1.execute('select `index`, sex,race,age_group,citizen,years_in_us,birthplace from base_init_lines2')
	### there are lot os null cells, this one has 6754 rows instead of around 117900
	##cur1.execute('select `index`, sex,race,age_group,citizen,years_in_us,birthplace,outgoing from base_init_lines2 where weight2001 is not null or weight2014 is not null or weight_new_people2001 is not null or weight_new_people2002 is not null or weight_new_people2003 is not null or weight_new_people2004 is not null or weight_new_people2005 is not null or weight_new_people2006 is not null or weight_new_people2007 is not null or weight_new_people2008 is not null or weight_new_people2009 is not null or weight_new_people2010 is not null or weight_new_people2011 is not null or weight_new_people2012 is not null or weight_new_people2013 is not null  or weight_new_people2014 is not null')
	cur1.execute('select `index`, sex,race,age_group,citizen,years_in_us,birthplace,outgoing,Visa,USFB, Slowdecrease, Fastdecrease, Activedecrease, p_10, HCW  from base_init_lines_class ')
	for row1 in cur1:
		dictval= [row1[1],row1[2],row1[3],row1[4],row1[5],row1[6],row1[7],row1[8],row1[9], row1[10] ,row1[11], row1[12], row1[13], row1[14]]
		Initial_Person_dict[row1[0]]=dictval

	con.close()
	#print "Initial_Person_dict ", Initial_Person_dict
...
def Make_TransitionProb_FromTo_Dict():
	...
	for val in xrange(1,75): ## the first 75 states are TB chanin states
		tpbasedict={}
...
def Make_TransitionProb_LTBI_FromTo_Dict():
	con = lite.connect('./limcat-zero-index.sqlite')
	cur1 = con.cursor()
	...
	for tostate in (1,2,3,30,63):
		stratdict={}
		cur1.execute('SELECT DISTINCT stratum_name, base from transition_probabilities_by_stratum_infection where from_state_id =0 and to_state_id= ' + str(tostate) )
		for row3 in cur1:
			#print "tostate ", tostate, str(row3[0]), float(row3[1])
			stratdict[str(row3[0])]=float(row3[1])
			TP_dict_ltbi[0,tostate] = stratdict


	con.close()
	#sstt='5 or more years Nicaragua Female Black Age 65-69'
	#print "len tpdictltbi", TP_dict_ltbi[0,1][sstt], TP_dict_ltbi[0,2][sstt],TP_dict_ltbi[0,3][sstt]
	return TP_dict_ltbi
...
def Make_TransitionProb_matrix():
	...
	#### modifying dictionary to remove droup-out state
	#counter=counter+1
	TPMatrix[12][0]=36
	TPMatrix[12][1]=[37,63,2]
	TPMatrix[12][2]=[0.899,6.08e-5,0.101]


	#counter=counter+1
	TPMatrix[13][0]=37
	TPMatrix[13][1]=[38,63,2]
	TPMatrix[13][2]=[0.899,6.08e-5,0.101]


	#counter=counter+1
	TPMatrix[14][0]=38
	TPMatrix[14][1]=[39,63,2]
	TPMatrix[14][2]=[0.899,6.08e-5,0.101]


	#print "---------------------TP matrix", TPMatrix
...
def Visa_readfile():
	f = open("Visa_Intervention.txt",'rU')
	temp=[]
	c=0
	counter=1;
	for fi in f:
...
def Make_Stratum_Name(pi, strataid):
	...
	if strataid == 0 or strataid == 1 or  strataid == 2 or strataid == 3 or strataid == 7:
		...
		Strstrata=  str(pi[1]) + ' ' + str(pi[34]) + ' ' + str(pi[2]) + ' ' + str(pi[3])
		# if pi[19] == 1:
...
def Make_Population_Sample_dict():
	con = lite.connect('./limcat-zero-index.sqlite')
	cur1 = con.cursor()
	cur2 = con.cursor()
	PopSample_dict={}

	cur1.execute('select `index`, weight2003 from base_init_lines_class where weight2003 is not null ')
	cur2.execute('select  sum(weight2003) from base_init_lines_class where weight2003 is not null ')
	...
	for row in cur1:
		...
		weights.append(float(row[1] /total)) ## get the percentage of weight for each index


	PopSample_dict['2003']=(elements, weights)


	cur1.execute('select `index`, weight2014 from base_init_lines_class where weight2014 is not null ')
	cur2.execute('select  sum(weight2014) from base_init_lines_class where weight2014 is not null ')
	...
	PopSample_dict['2014']=(elements, weights)


	#for i in range(2003,2015):
	for i in range(2001,2031):
		column='weight_new_people' + str(i)
		cur1.execute('select `index`, ' + column + ' from base_init_lines_class where ' + column + ' is not null ')
		cur2.execute('select  sum(' + column + ') from base_init_lines_class where ' + column + ' is not null ')
		...
		weights = []
		for row in cur1:
		#	print "---- ", i,column , row[0], row[1],  total, row[1] / float(total)
			elements.append(row[0])
			weights.append(row[1] / float(total)) ## get the percentage of weight for each index

		dictkey='new'+str(i)
		#print "dictkey", dictkey
		PopSample_dict[dictkey]=(elements, weights)
	con.close()
	#print "------ ", PopSample_dict
	return 	PopSample_dict

	#print " ==== PopSample_dict" , PopSample_dict['2001'][0],PopSample_dict['2001'][0]
...
#@return return the index and weight for each index of the base_init_lines2 table
#@brief this index values are used to assign the race, sex, ... for initial people
def Population_Sample(PopSample_dict,PopulationNumber,New,year):
	...
	print "======== Population sample", keyval
	elements=PopSample_dict[keyval][0]
	weights=PopSample_dict[keyval][1]

	#print "=== ", keyval,  sum(weights)
	#print "Weight ", New, year, len(elements), len(weights), weights
	#print "--- ", year, len(elements), weights
...
#@param Pop_Sample is the list created from "Population_Sample" function, containing the index of the people from "base_init_lines2" table
...
def Initial_Person_from_dict(P,ci, Pop_Sample,  Initial_People_Dict,Riskfactor_id,Riskfactor_name, Susceptible_ActiveCase_dict):
	...
	for i in xrange(len(P)):
		...
		P[i][29]=row[6]  #####
		# if ci > 155:
		# 	P[i][29]=row[9]  ### new outgoing range considering Hispanic FB
			#print "Hispanic FB rate ", row[9]
		P[i][30]=row[7]
		P[i][7]=row[8]
		P[i][31]=row[9]   #Slow latent decrease
		P[i][32]=row[10]   ### Fast latent decrease
		P[i][33]=row[11]  ## active cases
		P[i][35]= row[12]  ### prob of more than 10 years of ltbi
		#print "____ ", i, P[i][30] , P[i][2], P[i][6]
		#print "Check initial values , outgong , vias, slow latent, fast latent, active ", row[6], row[7], row[9], row[10], row[11]


		### ITs been set in the table
		...
		P[i][11]=0 #### months in current visa status
		...
		if r < row[13]:  ## probability of HCW
			#print "HCW+++++++++++++"
			P[i][37]=1
			P[i][36]=int(random.uniform(0,12))

	#jjj=np.where(P[:,11]==1)
	#print "Initilize peopel ", len(jjj[0])
...
def Set_InitialPerson_HIVRiskgroup(P):
	S_male=['IDU', 'MSM', 'General population men']
	T_male=[.005,.06,.935]


	S_female=['IDU','General population women']
	T_female=[.001,.999]

	for i in range(len(P)):
		if P[i][1] == 'Male':

			T=np.cumsum(T_male) ### cumulative sum
			...
			T=np.cumsum(T_female) ### cumulative sum
...
def Set_InitialPerson_Riskfactors(P,Riskfactor_id,Riskfactor_name, RiskFactor_StratumType_dict,  RiskFactor_TPvale_dict,ci):
	# global Riskfactor_id
	# global Riskfactor_name
	...
	for pi in xrange(len(P)):
		...
		for i in RiskFactor_StratumType_dict.keys():  ### loop over  riskfactors : stratum_type_id
			...
			#print "+++ ", i, RiskFactor_StratumType_dict.keys(), stratname
			keyval=(i,stratname)
			if keyval in RiskFactor_TPvale_dict.keys():
				...
				#### for calibration
				if i == 74 :
					#print "======SMOKING", stratname
					riskval= riskval * 1.17 * math.pow(.99,ci)


				if i == 183 :
					#print "======HIV stratname
					#riskval= riskval * 1.9
					riskval= riskval * 1.75


				if i == 83 :
					#print "======ESRD stratname
					riskval= riskval * .9
...
def Initializing_people(PopSample_dict, PopulationNumber,ci, Riskfactor_id,Riskfactor_name,  RiskFactor_TPvale_dict,  Susceptible_ActiveCase_dict,  RiskFactor_StratumType_dict,  Initial_People_Dict,Flagy):
	#cdef int i
	...
	P = np.zeros(shape=(PopulationNumber,39),  dtype=object)
	...
	#print "Here at initializing ", yearval, Flagy
	Pop_Sample=Population_Sample(PopSample_dict,PopulationNumber,Flagy,yearval)
	...
	P=Set_InitialPerson_HIVRiskgroup(P)

	P=Set_InitialPerson_Riskfactors(P,Riskfactor_id,Riskfactor_name, RiskFactor_StratumType_dict, RiskFactor_TPvale_dict,ci)
